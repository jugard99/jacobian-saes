...
from jacobian_saes.sae_pair import SAE_CFG_PATH, SAE_WEIGHTS_PATH, SPARSITY_PATH, SAEPair


def upload_saes_to_huggingface(
    saes_dict: dict[str, SAEPair | Path | str],
    hf_repo_id: str,
...
def _create_default_readme(repo_id: str, sae_ids: Iterable[str]) -> str:
    readme = dedent(
        ...
        # SAEs for use with the Jacobian SAE codebase
    ...
    readme += dedent(
        f"""
        Load these SAEs using Jacobian SAEs as below:
        ```python
        from jacobian_saes import SAEPair

        sae_pair = SAEPair.load_from_pretrained(path)
        ```
        """
    )
    return readme.strip()
...
def _build_sae_path(sae_ref: SAEPair | Path | str, tmp_dir: str) -> Path:
    if isinstance(sae_ref, SAEPair):
