...
def load_sae_from_local_path(path: str) -> Tuple[SAEPair, torch.Tensor]:
    sae = SAEPair.load_from_pretrained(path)
...
def get_gpt2_res_jb_saes(
    ...
    device: str = "cpu",
) -> tuple[dict[str, SAEPair], dict[str, torch.Tensor]]:
    """
    ...
    for hook_point in tqdm(GPT2_SMALL_RESIDUAL_SAES_HOOK_POINTS):
        ...
        sae = SAEPair.load_from_pretrained(folder_path, device=device)
...
def convert_connor_rob_sae_to_our_saelens_format(
    ...
    ae_alt = SAEPair.from_dict(config)
    ae_alt.load_state_dict(state_dict)
    return ae_alt


def get_gpt2_small_ckrk_attn_out_saes() -> dict[str, SAEPair]:
