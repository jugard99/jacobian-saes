...
class TrainingSAE(SAE):
    ...
    def __init__(
        ...
        mlp: Optional[TransformerLensMLP] = None,
    ):
        ...
        if cfg.use_jacobian_loss:
            ...
            assert mlp is not None and isinstance(
                mlp, TransformerLensMLP
            ), "You need to pass in an MLP into TrainingSAE to use the Jacobian loss"
            self.mlp = MLPWithActGrads(mlp.cfg)
    ...
    def encode_with_hidden_pre(
        self, x: Float[torch.Tensor, "... d_in"], return_topk_indices: Literal[False]
    ) -> tuple[Float[torch.Tensor, "... d_sae"], Float[torch.Tensor, "... d_sae"]]: ...

    @overload
    def encode_with_hidden_pre(
        self, x: Float[torch.Tensor, "... d_in"], return_topk_indices: Literal[True]
    ) -> tuple[
        Float[torch.Tensor, "... d_sae"],
        Float[torch.Tensor, "... d_sae"],
        Float[torch.Tensor, "... d_sae"],
    ...
    def encode_with_hidden_pre(
        self, x: Float[torch.Tensor, "... d_in"], return_topk_indices: bool = False
    ) -> (
        tuple[Float[torch.Tensor, "... d_sae"], Float[torch.Tensor, "... d_sae"]]
        | tuple[
            Float[torch.Tensor, "... d_sae"],
    ...
    def training_forward_pass(
        ...
        if self.cfg.use_jacobian_loss:
            feature_acts, _, topk_indices = self.encode_with_hidden_pre_fn(
                sae_in, return_topk_indices=True
            )
        else:
        ...
        if self.cfg.architecture == "gated":
            ...
            if self.cfg.use_jacobian_loss:
                jacobian_loss = (
                    current_jacobian_coefficient * self.calculate_jacobian_loss(sae_out, topk_indices)
                )
                loss = mse_loss + l1_loss + ghost_grad_loss + jacobian_loss
            else:
                jacobian_loss = torch.tensor(0.0)
    ...
    def calculate_jacobian_loss(
        self, sae_out: torch.Tensor, topk_indices: torch.Tensor
    ) -> torch.Tensor:
        wd1 = self.W_dec @ self.mlp.W_in  # (d_sae, d_mlp)
        w2e = self.mlp.W_out @ self.W_enc  # (d_mlp, d_sae)

        mlp_out, mlp_act_grads = self.mlp(sae_out)
        # TODO handle parallel attn and MLP
        resid_post = mlp_out + sae_out
        _, _, topk_indices2 = self.encode_with_hidden_pre_fn(
            resid_post, return_topk_indices=True
        )
